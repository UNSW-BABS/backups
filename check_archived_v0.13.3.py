...
add additional summary numbers?
  tab indented
  counts for each reason and within labelled age brackets
                        ...
                        if not os.access(local_path, os.R_OK):
                            ...
                            try:
                                ...
                                if remote_crc != local_crc and remote_crc != local_crc.lstrip('0') and remote_crc != '0' + local_crc.lstrip('0'):
                                    ...
                                    #print('cmd =', cmd)
                                    try:
                                        offline = subprocess.check_output(cmd, shell=True, stderr=subprocess.STDOUT).split('committed-to-tape')[1].strip(' "\n')
                                    except subprocess.CalledProcessError as err:
                                        ## Checks should prevent this occuring
                                        fail_counter += 1
